...
- Например мы случайно написали несколько элементов с одинаковым индексам, а элементов может быть много, то мы заметим странное поведение при рендеринге компонента, они будут постоянно менять свое место, то есть не будет стоять на месте
