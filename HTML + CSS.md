...
<details>
<summary>8.	Что такое псевдоэлементы, а также псевдоклассы и для чего они используются?</summary>
...
Название блока характеризует смысл («что это?» — «меню»: menu, «кнопка»: button). Блоку не следует задавать отступы, границы и позиционирование. 

Элемент - неотделимая часть блока, например пункты в списке меню.

Модификатор - сущность, определяющая внешний вид, состояние или поведение блока или элемента.

Например разный цвет кнопки в зависимости от темы.

С помощью БЭМ легко и быстро разрабатывать интерфейсы любой сложности и повторно использовать существующий код, избегая дублирования.
